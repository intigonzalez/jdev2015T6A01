...
mkdir encoding
cd encoding
ffmpeg -i $1 -c:v libx264 -profile:v main -level 3.1 -b:v ${BITRATE_HIGH}k -vf scale=$WIDTH3:$HEIGHT3 -c:a aac -strict -2 -force_key_frames expr:gte\(t,n_forced*4\) high.mp4
ffmpeg -i $1 -c:v libx264 -profile:v main -level 3.1 -b:v ${BITRATE_MEDIUM}k -vf scale=$WIDTH2:$HEIGHT2 -c:a aac -strict -2 -force_key_frames expr:gte\(t,n_forced*4\) mid.mp4
ffmpeg -i $1 -c:v libx264 -profile:v main -level 3.1 -b:v ${BITRATE_LOW}k -vf scale=$WIDTH1:$HEIGHT1 -c:a aac -strict -2 -force_key_frames expr:gte\(t,n_forced*4\) low.mp4
cd ..
# Chunks HLS
mkdir hls
cd hls
