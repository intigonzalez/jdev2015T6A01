...
public class RequestRelationServiceImpl implements RequestRelationService {
	...
	@Override
	public User get(String UserID, String UserToGet) throws IOException, NoSuchUserException,
			NoSuchBoxException {
	...
	@Override
	public void postRelation(Relation relationOfRequest, Relation relationToRequest)
			throws IOException, NoSuchBoxException {
		Box boxRelation = requestServ.getBox(relationToRequest.getEmail());
		WebTarget target = client.target(boxRelation.getIp() + "/api/app/" + relationToRequest.getEmail()
				+ "/relation/frombox");
